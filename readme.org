...
* Known Issues
1. For some reason, ipv6 sites will not work when using Chomper on some networks, but they will work when using Chomper on other networks. I have a theory that this is a Comcast/Xfinity-related issue, but I'm not completely sure.
